...
RamPage	= $10												; VISICOM: data RAM is at $1000
VideoPage = $11											; VISICOM: display is at $1100
ColourPage = $13											; VISICOM: the second bit plane
...
PX_NoCollideLeftPart:
		...
		xor 	
		str 	rf
		glo 	rc 										; COLOUR: object records are 6 bytes from $00, and RC is
		smi 	$11 										; parked at base+5 here, so base >= $0C -- RC.0 >= $11 --
		bnf 	_PXTintA 								; means object 2 or later: a robot or a robot's shot.
		ghi 	rf 										; mirror the same pattern into plane 1, which turns it
		adi 	2 										; red. Player (object 0) and his missile (1) stay cyan,
		phi 	rf 										; and so do the walls.
		glo 	re
		xor
		str 	rf
		ghi 	rf
...
PX_NoCollideRightPart:
		...
		glo 	rc 										; COLOUR: object records are 6 bytes from $00, and RC is
		smi 	$11 										; parked at base+5 here, so base >= $0C -- RC.0 >= $11 --
		bnf 	_PXTintB 								; means object 2 or later: a robot or a robot's shot.
		ghi 	rf 										; mirror the same pattern into plane 1, which turns it
		adi 	2 										; red. Player (object 0) and his missile (1) stay cyan,
		phi 	rf 										; and so do the walls.
		ghi 	re
...
PX_NoCollideSingle:											
		...
		glo 	rc 										; COLOUR: object records are 6 bytes from $00, and RC is
		smi 	$11 										; parked at base+5 here, so base >= $0C -- RC.0 >= $11 --
		bnf 	_PXTintC 								; means object 2 or later: a robot or a robot's shot.
		ghi 	rf 										; mirror the same pattern into plane 1, which turns it
		adi 	2 										; red. Player (object 0) and his missile (1) stay cyan,
		phi 	rf 										; and so do the walls.
		glo 	re
